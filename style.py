...
def check_opts(opts):
    ...
    exists(opts.train_path, "train2014 path not found!")
    if opts.test:
...
def main():
    ...
    for style_file in style_files:
        print("-------------Started to train2014 model for style '%s'" % os.path.basename(style_file))
        style_target = get_img(style_file)
        checkpoint_dir = "checkpoint_" + os.path.splitext(os.path.basename(style_file))[0]
